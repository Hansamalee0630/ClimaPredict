...
def plot_forecast_chart(col_key, color, title, unit, warn_val, danger_val):
    ...
    # THARU'S FIX: Explicit EPA Mold Limit Line
    if title == "Humidity":
        fig.add_hline(y=60, line_dash="dash", line_width=3, line_color="#ff5e7e")
        fig.add_annotation(
            x=0.5, y=60, 
            xref="paper", yref="y", # <-- THIS FIXES THE 1970 BUG!
            text="EPA Mold Risk Limit (60% RH)", 
            showarrow=False, 
            font=dict(color="#ff5e7e", size=13),
            yanchor="bottom"
        )
...
with tab_overview:
    ...
    # OSHI'S FIX: Network Status Table with Last Seen Timestamp
    st.markdown('<div class="sh" style="margin-top: 40px;">System Health & Network Topology</div>', unsafe_allow_html=True)
    st.markdown("""
    <table style="width:100%; border-collapse:collapse; font-size:14px; background:var(--bg1); border-radius:12px; overflow:hidden;">
        <tr style="border-bottom:2px solid #1f3c5b; color:#93b9d6; text-align:left; background:var(--bg2);">
            <th style="padding:12px;">Device ID</th><th style="padding:12px;">Location</th><th style="padding:12px;">Status</th><th style="padding:12px;">Last Seen Timestamp (Fix)</th>
        </tr>
        <tr style="border-bottom:1px solid #1f3c5b;">
            <td style="padding:12px;">ENS160_Room402</td><td style="padding:12px;">Main Lab</td><td style="padding:12px;"><span style="color:#2ecc71; font-weight:bold;">● Online</span></td><td style="padding:12px; font-family:monospace;">Just now</td>
        </tr>
        <tr style="border-bottom:1px solid #1f3c5b;">
            <td style="padding:12px;">GP2Y10_Hallway</td><td style="padding:12px;">Corridor B</td><td style="padding:12px;"><span style="color:#e74c3c; font-weight:bold;">● Offline</span></td><td style="padding:12px; font-family:monospace;">45 mins ago</td>
        </tr>
    </table>
    """, unsafe_allow_html=True)
...
with tab_air:
    ...
    # MILI'S FIX: Dynamic Actionable Banner (Connected to Sliders)
    if last_reading['co2'] >= st.session_state.co2_d:
        st.markdown('<div style="padding:15px; background:rgba(231,76,60,0.15); border:2px solid #e74c3c; color:#e74c3c; border-radius:8px; margin-bottom:15px;"><b>🚨 HAZARD: Cognitive decline active. Vacate or ventilate immediately.</b></div>', unsafe_allow_html=True)
    elif last_reading['co2'] >= st.session_state.co2_w:
        st.markdown('<div style="padding:15px; background:rgba(241,196,15,0.15); border:2px solid #f1c40f; color:#f1c40f; border-radius:8px; margin-bottom:15px;"><b>⚠️ Action Recommended: Open Windows for 10 Minutes.</b></div>', unsafe_allow_html=True)
...
with tab_forecast:
    ...
    if fore_param:
        ...
        # # HANSA'S FIX: CSV Export Button for Anomalies
        csv_data = df_combined.to_csv(index=False).encode('utf-8')
        st.download_button("📥 Export Anomaly Data to CSV", data=csv_data, file_name="clima_predict_export.csv", mime="text/csv", type="primary")